...
//==============================================================================
MPU9250::MPU9250(unsigned char i2cbus, int i2caddress)
{
  I2CBus = i2cbus;
...
void MPU9250::updateTime()
{
  ...
  deltat = (Now - lastUpdate)/CLOCKS_PER_SEC;
  lastUpdate = Now;
...
// Configure MPU6050 gyro and accelerometer for bias calculation
  ...
  sleep(0.04); // accumulate 40 samples in 40 milliseconds = 480 bytes
...
// Configure the accelerometer for self-test
  ...
  sleep(0.025);  // sleep a while to let the device stabilize
...
*/  
  ...
  char I2CBuffer[40];
  snprintf(I2CBuffer, 30,"/dev/i2c-%d", I2CBus);
  I2CFile= open(I2CBuffer, O_RDWR);
  if(I2CFile == -1)
  {
  ...
  i2c_smbus_write_word_data(I2CFile, subAddress, data);
...
*/
  char n[10];
  char I2CBuffer[11];
  snprintf(I2CBuffer, 11,"/dev/i2c-%d", I2CBus);
  ...
  uint8_t value= (uint8_t) i2c_smbus_read_byte_data(I2CFile, subAddress);
...
*/
  char I2CBuffer[25];
  snprintf(I2CBuffer, 25,"/dev/i2c-%d", I2CBus);
  I2CFile= open(I2CBuffer, O_RDWR);
  if(I2CFile == -1)
  {
    printf("Something went wrong with open()! %s\n", strerror(errno));
  ...
  while(count >0)
  {
    dest[i++]= (uint8_t) i2c_smbus_read_byte_data(I2CFile, subAddress);
    count--;
  }
